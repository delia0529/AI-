...
### 朗读播报（浏览器本地合成，无云端服务、无预生成音频）

- **入口**：右上角「朗读 Read」读全文；每个模块标题旁的「朗读本节」只读该模块
- **控制条**：暂停 / 继续、停止、语速（0.9×–1.5×）、可拖动的进度条
- **章节锚点**：进度条上标注每个模块起点，当前章节锚点加粗高亮，点击锚点直接跳章
- **锚定跟随**：拖动或跳转时对应正文高亮并滚到视口中央，右侧显示当前段文字预览
- 只读标题 / 核心事实 / 深度洞察，跳过来源链接与图注；切走页面自动停止
